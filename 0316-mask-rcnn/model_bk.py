...
import FPN
import ResNet_bk as ResNet
import proposal
import RPN
import FPN_head_bk as FPN_head
import detection_layer
import detection_target_layer
import loss_layer
import utils_log
import data_generator_layer
...
class MaskRCNN(nn.Module):
    ...
    def build(self, config):
        ...
        # RPN
        self.rpn = RPN.RPN(len(config.RPN_ANCHOR_RATIOS), config.RPN_ANCHOR_STRIDE, 256)

        # FPN Classifier
        self.classifier = FPN_head.Classifier(256, config.POOL_SIZE, config.IMAGE_SHAPE, config.NUM_CLASSES)

        # FPN Mask
        self.mask = FPN_head.Mask(256, config.MASK_POOL_SIZE, config.IMAGE_SHAPE, config.NUM_CLASSES)
    ...
    def predict(self, input, mode):
        ...
        rpn_rois = proposal.proposal_layer([rpn_class, rpn_bbox],
        ...
        if mode == 'inference':
            ...
            detections = detection_layer.detection_layer(self.config, rpn_rois, mrcnn_class, mrcnn_bbox, image_metas)
            ...
            rois, target_class_ids, target_deltas, target_mask = \
                detection_target_layer.detection_target_layer(rpn_rois, gt_class_ids, gt_boxes, gt_masks, self.config)
    ...
    def train_epoch(self, datagenerator, optimizer, steps):
        ...
        for inputs in datagenerator:
            ...
            # Compute losses
            rpn_class_loss, rpn_bbox_loss, mrcnn_class_loss, mrcnn_bbox_loss, mrcnn_mask_loss = loss_layer.compute_losses(rpn_match, rpn_bbox, rpn_class_logits, rpn_pred_bbox, target_class_ids, mrcnn_class_logits, target_deltas, mrcnn_bbox, target_mask, mrcnn_mask)
            ...
            # Progress
            utils_log.printProgressBar(step + 1, steps, prefix="\t{}/{}".format(step + 1, steps),
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            molded_image = utils_data_format.mold_image(molded_image, self.config)
            # Build image_meta
            image_meta = utils_data_format.compose_image_meta(
