...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, skip_connection, deep_mask=None):
        ...
        self.mask0 = uitls.conv1x1(64, 1)
        if deep_mask is not None:
            self.mask4 = uitls.conv1x1(1024, 1)
            self.mask3 = uitls.conv1x1(512, 1)
            self.mask2 = uitls.conv1x1(256, 1)
            self.mask1 = uitls.conv1x1(64, 1)
    ...
    def forward(self, x):
        ...
        x = self.skip2((x, skip))
        mask2 = self.mask2(x)
        mask2 = self.Sigmoid(mask2)

        skip = skip_connect.pop()
        # 56
        x = self.skip1((x, skip))
        mask1 = self.mask1(x)
        mask1 = self.Sigmoid(mask1)

        skip = skip_connect.pop()
        # 112
        x = self.skip0((x, skip))
        x = self.mask0(x)
        mask0 = self.Sigmoid(x)
