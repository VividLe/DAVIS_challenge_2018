...
class Saliency(data.Dataset):
    ...
    def __init__(self, root, split='train', joint_transform=None, transform=None):
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(path)
        path_cont = path.replace('.jpg', '.png')
        img_cont = Image.open(path_cont)
        # positive exemplar
        pos_path = path.replace(self.split, self.split + 'pos')
        pos = Image.open(pos_path)
        pos_cont_path = pos_path.replace('.jpg', '.png')
        pos_cont = Image.open(pos_cont_path)
        # negative exemplar
        neg_path = path.replace(self.split, self.split + 'neg')
        neg = Image.open(neg_path)
        neg_cont_path = neg_path.replace('.jpg', '.png')
        neg_cont = Image.open(neg_cont_path)

        if self.joint_transform is not None:
            img, img_cont, pos, pos_cont, neg, neg_cont = self.joint_transform([img, img_cont,
                                                  pos, pos_cont,
                                                  neg, neg_cont])
        ...
        if self.transform is not None:
            ...
            pos_cont = self.transform(pos_cont)
            neg_cont = self.transform(neg_cont)

        return img, img_cont, pos, pos_cont, neg, neg_cont
