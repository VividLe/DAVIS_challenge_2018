...
class LargeKernelLayer(nn.Sequential):
	def __init__(self, channels):
		super(LargeKernelLayer, self).__init__()
		self.add_module('norm', nn.BatchNorm2d(num_features=channels))
		# self.add_module('relu', nn.ReLU(inplace=True))
		self.add_module('prelu', nn.PReLU())

		# author's impl - lasange 'same' pads with half
		# filter size (rounded down) on "both" sides
		self.add_module('conv', nn.Conv2d(in_channels=channels,
										  out_channels=channels, kernel_size=7, stride=1,
										  padding=3, bias=False))

		self.add_module('drop', nn.Dropout2d(0.2))

	def forward(self, x):
		return super(LargeKernelLayer, self).forward(x)
...
class TransitionUp(nn.Module):
	def __init__(self, in_channels, out_channels):
		...
		self.convTrans = nn.ConvTranspose2d(in_channels=in_channels,
											...
											padding=0,
											bias=False)  # crop = 'valid' means padding=0. Padding has reverse effect for transpose conv (reduces output size)
		self.layers = nn.ModuleList([LargeKernelLayer(out_channels)])
	...
	def forward(self, x, skip):
		...
		# convolution with 7x7 twice
		large_conv_layer = self.layers[0]
		out = large_conv_layer(out)
		out = large_conv_layer(out)
...
class FCDenseNet(nn.Module):
	def __init__(self, in_channels=3, down_blocks=(5, 5, 5, 5, 5),
		...
		#      Softmax      #
		#####################
		self.deepSup = nn.Conv2d(in_channels=48,
	...
	def forward(self, inputs):
		...
		for i in range(len(self.up_blocks)):
			...
			if i < len(self.up_blocks) - 1:
				out_mask.append(self.deepSup(out))
...
def FCDenseNet57(in_channels=3, n_classes=2):
    return FCDenseNet(in_channels=in_channels, down_blocks=(4, 4, 4, 4, 4),
                 up_blocks=(4, 4, 4, 4, 4), bottleneck_layers=4,
                 growth_rate=12, out_chans_first_conv=48, n_classes=n_classes)

def FcDnSmall(in_channels=3, n_classes=2):
    return FCDenseNet(in_channels=in_channels, down_blocks=(4, 4, 4, 4),
